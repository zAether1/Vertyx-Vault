...
export default function ProfileStudio() {
  ...
  if (!session.profile) return <section className="vault-auth-card"><div className="vault-auth-card__brand"><img src="/Vertyx-Vault-2.png" alt="Vertyx Vault" /></div><span className="vault-page__eyebrow">Acceso seguro</span><h2>Entra a tu cuenta</h2><p>Continúa con un proveedor para iniciar sesión o crear tu cuenta. No creamos ningún perfil hasta que el proveedor confirme tu identidad.</p><div className="vault-auth-card__providers"><button type="button" disabled={busy} onClick={() => continueWithProvider('google')}><img src="/social-logo.svg" alt="" aria-hidden="true" />Continuar con Google</button><button type="button" disabled={busy} onClick={() => continueWithProvider('discord')}><img src="/discord-logo.svg" alt="" aria-hidden="true" />Continuar con Discord</button><button type="button" className="vault-auth-card__guest" disabled={busy} onClick={() => signIn('guest')}><img src="/Guest-Logo.png" alt="" aria-hidden="true" />Explorar como invitado</button></div>{notice && <p className="vault-profile-notice">{notice}</p>}<small>Al continuar aceptas los términos y la política de privacidad de Vertyx.</small></section>;
