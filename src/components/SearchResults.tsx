...
export default function SearchResults({ id, state }: SearchResultsProps) {
  ...
  return (
    ...
    >
      {state.status === 'empty' && <p className="text-white p-3">No hay coincidencias en el catálogo actual.</p>}
      {state.status === 'results' && state.items.map((item) => {
        const title = item.title || item.name || 'Sin título';
        return (
          <a key={item.id} href={searchItemLink(item)} className="flex items-center p-3 hover:bg-white/10 transition-colors border-b border-white/10 last:border-0">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={item.poster ?? '/vertyx-mark.svg'} alt="" className="w-12 h-[72px] object-cover rounded-md bg-white/5" />
            <div className="ml-3 flex-1 min-w-0">
              <h3 className="text-white font-medium truncate">{title}</h3>
              <p className="text-white/65 text-sm mt-1 truncate">{item.overview}</p>
