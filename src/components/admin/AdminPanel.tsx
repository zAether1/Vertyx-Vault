...
import type { AdminOverview, ModerationResult } from '@/types/infrastructure';
...
function SubmissionReview({ item, onModerate }: { item: ContentSubmission; onModerate: (id: string, action: 'approve' | 'reject') => void }) {
  return <article className="vault-admin-submission">
    ...
    <dl><div><dt>Categoría</dt><dd>{item.category}</dd></div><div><dt>Año</dt><dd>{item.year || '—'}</dd></div><div><dt>Idioma</dt><dd>{item.language || '—'}</dd></div><div><dt>Calidad</dt><dd>{item.quality || '—'}</dd></div><div><dt>Proveedor</dt><dd>{item.provider}</dd></div><div><dt>Formato</dt><dd>{item.playbackKind.toUpperCase()}</dd></div><div className="vault-admin-submission__wide"><dt>Géneros</dt><dd>{item.genres.join(', ') || '—'}</dd></div>{item.notes && <div className="vault-admin-submission__wide"><dt>Observaciones</dt><dd>{item.notes}</dd></div>}</dl>
    <div className="vault-admin-submission__actions"><button type="button" className="vault-admin-button vault-admin-button--approve" onClick={() => onModerate(item.id, 'approve')}>Aceptar</button><button type="button" className="vault-admin-button vault-admin-button--reject" onClick={() => onModerate(item.id, 'reject')}>Rechazar</button></div>
  </article>;
}
