...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const body = await request.json().catch(() => ({})) as { playbackUrl?: string; playbackKind?: 'hls' | 'mp4' | 'dash' | 'embed' };
  ...
  return updateSubmissionPlayback(request, id, body.playbackUrl, body.playbackKind);
}
