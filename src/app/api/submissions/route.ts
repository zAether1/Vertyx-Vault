...
import { createSubmission, hasDatabase, listSubmissions } from '@/server/database/repositories';
...
export async function GET(request: Request) {
  ...
  if (hasDatabase()) {
    try {
      const items = await listSubmissions(profile.id, can(profile.role, 'submission:review'));
      if (items) return NextResponse.json({ items });
    } catch (error) {
      console.error('[submissions-database]', error);
    }
  }
...
export async function POST(request: Request) {
  ...
  if (hasDatabase()) {
      ...
      console.error('[submissions-database:save]', error);
    }
  }
